...
/**
 * Renders the saved-cafés list (empty state, rows, notes). Used inside the header dropdown.
 */
export function SavedPlacesPanel() {
  ...
  if (count === 0) {
    return <p className={styles.empty}>{t('saved.empty')}</p>;
  }
  ...
  return (
    <>
      ...
      {coffeeShops.length > 0 ? <p className={styles.focusHint}>{t('saved.focusHint')}</p> : null}
    </>
  );
}
